...
def extract_data(xml_file):
    ...
    for rejection_stats in root.findall('.//auth:RjctnSttstcs', NAMESPACES):
        ...
        responsible_entity_lei = get_text(rejection_stats.find('.//auth:NttyRspnsblForRpt/auth:LEI', NAMESPACES))
        ...
        for rejection_reason in rejection_stats.findall('.//auth:TxsRjctnsRsn', NAMESPACES):
            record = {
                ...
                'OtherCounterparty_LEI': get_text(
                    rejection_reason.find('.//auth:TxId/auth:OthrCtrPty/auth:Lgl/auth:Id/auth:Othr/auth:Id/auth:Id',
                                          NAMESPACES)),
                ...
                'ValidationRuleScheme': get_text(
                    rejection_reason.find('.//auth:DtldVldtnRule/auth:SchmeNm/auth:Cd', NAMESPACES))
            }
...
def insert_data_to_db(data, connection_string, batch_size=1000):
    ...
    INSERT INTO UnavistaMarex_Rejection_Statistics_Report (
        RefDate, ReportingCounterparty_LEI, SubmittingEntity_LEI, EntityResponsibleForReporting_LEI,
        ActionType, ReportingTimestamp, DerivativeEventTimestamp, OtherCounterparty_LEI,
        CollateralPortfolioCode, RejectionStatus, ValidationRuleId, ValidationRuleDesc, ValidationRuleScheme
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    '''
    try:
        with pyodbc.connect(connection_string) as conn:
            ...
            for row in data:
                batch.append((
                    row['RefDate'], row['ReportingCounterparty_LEI'], row['SubmittingEntity_LEI'],
                    row['EntityResponsibleForReporting_LEI'], row['ActionType'], row['ReportingTimestamp'],
                    row['DerivativeEventTimestamp'], row['OtherCounterparty_LEI'],
                    row['CollateralPortfolioCode'], row['RejectionStatus'], row['ValidationRuleId'],
                    row['ValidationRuleDesc'], row['ValidationRuleScheme']
                ))
                count += 1
